...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename(req, file, done) { // 제로초.png를 업로드
      const ext = path.extname(file.originalname); // .png(확장자를 추출)
      const basename = path.basename(file.originalname, ext); // 제로초(파일명 추출)와 확장자를 분리
      done(null, basename + '_' + new Date().getTime() + ext); // 제로초123123.png
    },
  }),
...
router.delete('/:postId/like', isLoggedIn, async (req, res, next) => { // DELETE /post/1/unlike
  try {
    const post = await Post.findOne({ where : { id: req.params.postId }});
    if (!post) {
      return res.status(404).send('게시물이 존재하지 않습니다.');
